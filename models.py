...
from utils import get_fft_feature, get_fft_feature2
...
class Generator_N2C(nn.Module):
    ...
    def forward(self, x, instance_norm=False):
        x_detach = x.detach()
        x = self.sub_mean(x)
        x = self.entry(x)
        c0 = o0 = x
...
class Spectral_Discriminator(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        # az_fft_feature_clone = az_fft_feature.clone()
        # az_fft_feature_clone[torch.isnan(az_fft_feature_clone)] = 0

        az_fft_feature = get_fft_feature2(input)
        
        return self.linear(az_fft_feature_clone[:,-self.thresh:])
...
if __name__ == '__main__':
    ...
    d = Discriminator()
    input_size = (3, 32, 32)
    print(summary(d, input_size, batch_size=-1, device='cpu'))
